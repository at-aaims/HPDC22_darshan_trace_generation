...
def pot_1(z):
    ...
    inner_term_1 = torch.exp((-0.5 * ((z_1 - 2) / 0.6) ** 2))
    inner_term_2 = torch.exp((-0.5 * ((z_1 + 2) / 0.6) ** 2))
...
def pot_2(z):
    u = 0.5 * ((z[:, 1] - w_1(z)) / 0.4) ** 2
    return -u


def pot_3(z):
    term_1 = torch.exp(-0.5 * ((z[:, 1] - w_1(z)) / 0.35) ** 2)
    term_2 = torch.exp(-0.5 * ((z[:, 1] - w_1(z) + w_2(z)) / 0.35) ** 2)
    u = -torch.log(term_1 + term_2 + 1e-7)
    return -u


def pot_4(z):
    term_1 = torch.exp(-0.5 * ((z[:, 1] - w_1(z)) / 0.4) ** 2)
    term_2 = torch.exp(-0.5 * ((z[:, 1] - w_1(z) + w_3(z)) / 0.35) ** 2)
